...
void FastText::supervised(Model& model, real lr,
                          ...
                          const std::vector<int32_t>& titles) {
  if (labels.size() == 0 || line.size() == 0) return;
 std::uniform_int_distribution<> uniform(0, labels.size() - 1);
  int32_t i = uniform(model.rng);
  for (auto it = labels.cbegin(); it != labels.cend(); ++it) {
    //std::cout<<"label::"<<labels[*it]<<std::endl;
    model.update(line, titles,*it, lr);
  }
  //model.update(line, titles, labels[i], lr);
}
...
void FastText::test(std::istream& in, int32_t k) {
  ...
  std::vector<int32_t> line, labels,titles;

  while (in.peek() != EOF) {
    dict_->getLine(in, line, labels, titles,model_->rng);
    if (labels.size() > 0 && line.size() > 0) {
      std::vector<std::pair<real, int32_t>> modelPredictions;
	  //std::cout<<labels.size()<<std::endl;
      model_->predict(line,titles, k, modelPredictions);
...
void FastText::predict(std::istream& in, int32_t k,
  ...
  std::vector<int32_t> words, labels, titles;
  predictions.clear();
  dict_->getLine(in, words, labels, titles, model_->rng);
  if (words.empty()) return;
  Vector hidden(args_->dim);
  Vector output(dict_->nlabels());
  std::vector<std::pair<real,int32_t>> modelPredictions;
  model_->predict(words, titles, k, modelPredictions, hidden, output);
  for (auto it = modelPredictions.cbegin(); it != modelPredictions.cend(); it++) {
...
void FastText::textVectors() {
  ...
  while (std::cin.peek() != EOF) {
    dict_->getLine(std::cin, line, labels, titles, model_->rng);
    vec.zero();
...
void FastText::trainThread(int32_t threadId) {
  ...
  while (tokenCount < args_->epoch * ntokens) {
    ...
    real lr = args_->lr * (1.0 - progress);
    localTokenCount += dict_->getLine(ifs, line, labels, titles, model.rng);
	//std::cout<<line.size()<<" "<<titles.size()<<std::endl;
    if (args_->model == model_name::sup) {
      supervised(model, lr, line, labels, titles);
...
void FastText::loadVectors(std::string filename) {
  ...
  dict_->threshold(1, 0);
  input_ = std::make_shared<Matrix>(dict_->nwords()+dict_->ntitles()+args_->bucket, args_->dim);
  input_->uniform(1.0 / args_->dim);

  for (size_t i = 0; i < n; i++) {
    int32_t idx = dict_->getId(words[i]);
    if (idx < 0 || idx >= (dict_->nwords() + dict_-> ntitles())) continue;
    for (size_t j = 0; j < dim; j++) {
...
void FastText::train(std::shared_ptr<Args> args) {
  ...
  if (args_->pretrainedVectors.size() != 0) {
	...
	if (args_->online == 0) {
		...
		std::ifstream ifs1(args_->modeli);
		input_ = std::make_shared<Matrix>();
		input_->load(ifs1);
		ifs1.close();
	}
  ...
  if (args_->online == 0) {
    ...
    std::ifstream ifs2(args_->modelo);
	output_ = std::make_shared<Matrix>();
	output_->load(ifs2);
	ifs2.close();
  }
  ...
  if (args_->online == 0) {
  	std::string fn1(args_->modeli);
    fn1 += ".bin";
    std::ofstream ofs1(fn1, std::ofstream::binary);
    if (!ofs1.is_open()) {
      std::cerr << "Model file cannot be opened for saving!" << std::endl;
      exit(EXIT_FAILURE);
    }
	...
	std::string fn2(args_->modelo);
    fn2 += ".bin";
    std::ofstream ofs2(fn2, std::ofstream::binary);
    if (!ofs2.is_open()) {
      std::cerr << "Model file cannot be opened for saving!" << std::endl;
      exit(EXIT_FAILURE);
    }
