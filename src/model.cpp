...
void Model::computeHidden(const std::vector<int32_t>& input, 
                          const std::vector<int32_t>& title_inputs, 
                          Vector& hidden) const {
  ...
  hidden.mul((1-TITLE_WEIGHT) / TITLE_WEIGHT);
  for (auto it = title_inputs.cbegin(); it != title_inputs.cend(); ++it) {
    if(quant_) {
      hidden.addRow(*qwi_, *it);
    } else {
      hidden.addRow(*wi_, *it);
  ...
  hidden.mul(TITLE_WEIGHT / (input.size() + title_inputs.size()));
...
void Model::predict(const std::vector<int32_t>& input, const std::vector<int32_t>& title, int32_t k,
                    std::vector<std::pair<real, int32_t>>& heap,
                    Vector& hidden, Vector& output) const {
  assert(k > 0);
  heap.reserve(k + 1);
  computeHidden(input, title, hidden);
...
void Model::predict(const std::vector<int32_t>& input, const std::vector<int32_t>& title,  int32_t k,
                    std::vector<std::pair<real, int32_t>>& heap) {
  predict(input, title, k, heap, hidden_, output_);
}
...
void Model::update(const std::vector<int32_t>& input, const std::vector<int32_t>& title_inputs, int32_t target, real lr) {
  //std::cout<<target<<" "<<osz_<<std::endl;
  assert(target >= 0);
  assert(target < osz_);
  //std::cout<<"Input Size: "<<input.size()<<" Title Size: "<<title_inputs.size()<<std::endl;
  if (input.size() == 0) return;
  if (title_inputs.size() == 0) return;
  computeHidden(input,title_inputs, hidden_);
