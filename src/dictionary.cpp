...
Dictionary::Dictionary(std::shared_ptr<Args> args) : args_(args),
  word2int_(MAX_VOCAB_SIZE, -1), size_(0), nwords_(0), nlabels_(0),ntitles_(0),
  ntokens_(0) {}
...
void Dictionary::add(std::string& w) {
  bool isTitle=false;
  if(getType(w) == entry_type::title)
  {
	//std::cout<<w<<std::endl;
    w=w.substr(9);
	isTitle=true;
...
int32_t Dictionary::ntitles() const {
  return ntitles_;
}
...
entry_type Dictionary::getType(const std::string& w) const {
  return (w.find(args_->label) == 0)  ? entry_type::label : (w.find(args_->title) == 0) ? entry_type::title : entry_type::word;
}
...
void Dictionary::readFromFile(std::istream& in) {
  ...
  if (args_->verbose > 0) {
	...
	std::cerr << "Number of titles: " << ntitles_ << std::endl;
  }
  if (size_ == 0) {
...
void Dictionary::threshold(int64_t t, int64_t tl) {
  ...
  for (auto it = words_.begin(); it != words_.end(); ++it) {
    ...
    if (it->type == entry_type::title) ntitles_++; 
  }
}
...
int32_t Dictionary::getLine(std::istream& in,
  ...
  while (readWord(in, token)) {
  	bool isTitle=false;
    if(getType(token) == entry_type::title)
    {
    ...
    if (wid < 0) {
      ...
      if (type == entry_type::word || isTitle) word_hashes.push_back(hash(token));
      continue;
    }
    ...
    if (type == entry_type::label) {
      labels.push_back(wid - nwords_ - ntitles_);
    }
    if (isTitle /*&& !discard(wid, uniform(rng))*/) {
      titles.push_back(wid);
     // word_hashes.push_back(hash(token));
    }
    if (token == EOS) break;
...
int32_t Dictionary::getLine(std::istream& in,
                            ...
                            std::minstd_rand& rng) const {
  std::vector<int32_t> word_hashes;
  int32_t ntokens = getLine(in, words, word_hashes, labels,titles, rng);
...
std::string Dictionary::getLabel(int32_t lid) const {
  assert(lid >= 0);
  assert(lid < nlabels_);
  return words_[lid + nwords_ + ntitles_].word;
}
...
void Dictionary::save(std::ostream& out) const {
  ...
  out.write((char*) &ntitles_, sizeof(int32_t));
...
void Dictionary::load(std::istream& in) {
  ...
  in.read((char*) &ntitles_, sizeof(int32_t));
