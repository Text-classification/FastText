...
void Args::parseArgs(int argc, char** argv) {
  ...
  while (ai < argc) {
    ...
    } else if (strcmp(argv[ai], "-modelo") == 0) {
	  modelo = std::string(argv[ai + 1]);
	} else if (strcmp(argv[ai], "-modeli") == 0) {
	  modeli = std::string(argv[ai + 1]);
	} else if (strcmp(argv[ai], "-online") == 0) {
	  online = atof(argv[ai + 1]);
